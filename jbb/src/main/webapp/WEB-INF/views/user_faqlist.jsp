...
<style>
  ...
  *{ margin:0; padding:0; box-sizing:border-box; }
  ...
  .board-item.faq-toggle{
    cursor: default;
  }

  .board-item.faq-toggle .board-content-wrapper{
    cursor: pointer;
  }
...
<div class="container">
    <div class="page-hero">
      <div>
        <div class="hero-title">자주 묻는 질문을 한눈에 확인해보세요</div>
        <div class="hero-desc">궁금한 내용을 검색하고, 질문을 눌러 바로 답변까지 확인할 수 있어요.</div>
      </div>
      <div class="hero-pill">❓ FAQ 모아보기</div>
    </div>
  <!-- 검색 기능 -->
  <div class="search-container">
    <form action="<c:url value='/faq/user_faqlist'/>" class="search-form" method="get">
      <input type="hidden" name="page" value="1">
      <input type="hidden" name="pageSize" value="${sc.pageSize}">
      <select class="search-option" name="option">
        <option value="Q" ${sc.option == 'Q' ? "selected" : ""}>질문</option>
      </select>
  ...
  <div class="grid-container">
    <c:choose>
      <c:when test="${empty list}">
        <div class="nolist">해당하는 FAQ가 없습니다.</div>
      </c:when>
      <c:otherwise>
        <c:forEach var="faqDto" items="${list}">
          <div class="board-item faq-toggle">
            <div class="main_img">
            ...
            <div class="board-content-wrapper">
              ...
              <p class="board-content">
                <c:choose>
                  <c:when test="${fn:length(faqDto.f_answer) > 25}">
                    ${fn:substring(faqDto.f_answer, 0, 25)}...
                  </c:when>
                  <c:otherwise>
                    ${faqDto.f_answer}
                  </c:otherwise>
              ...
              <div class="board-bottom">
                <p class="b-b"><fmt:formatDate value="${faqDto.f_date}" pattern="yyyy. M. dd"/></p>
              </div>
            </div>
          ...
          <div class="faq-detail">
            <pre>${faqDto.f_answer}</pre>
            <span class="faq-date">
              <fmt:formatDate value="${faqDto.f_date}" pattern="yyyy. M. dd"/>
            </span>
          </div>
        </c:forEach>
      </c:otherwise>
    ...
    <div class="paging">
      <c:if test="${totalCnt != null && totalCnt != 0}">
        <c:if test="${ph.showPrev}">
          <a class="page" href="<c:url value='/faq/user_faqlist${ph.sc.getQueryString(ph.beginPage-1)}'/>">&lt;</a>
        </c:if>

        <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
          <a class="page ${i==ph.sc.page ? 'paging-active' : ''}"
             href="<c:url value='/faq/user_faqlist${ph.sc.getQueryString(i)}'/>">${i}</a>
        </c:forEach>

        <c:if test="${ph.showNext}">
          <a class="page" href="<c:url value='/faq/user_faqlist${ph.sc.getQueryString(ph.endPage+1)}'/>">&gt;</a>
        </c:if>
      </c:if>
    </div>
  </div>
...
$(document).ready(function () {
  $(".board-item.faq-toggle").click(function (e) {
  ...
  $(".toggle-icon").click(function (e) {
    ...
    var content = $(this).closest(".board-item").next(".faq-detail");
    var icon = $(this);
