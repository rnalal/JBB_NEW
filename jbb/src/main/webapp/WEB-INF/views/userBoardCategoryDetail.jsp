...
<div class="top-bar">
	...
	<div class="search-container">
		<form action="<c:url value="/admin/board-category-detail"/>" class="search-form" method="get">
			<input type="hidden" name="bc_name" value="${ph.sc.bc_name}">
...
<div class="content-container">	
	...
	<div class="container">		
		    <span class="alluser"><span class="tit">${ph.sc.bc_name}</span> 카테고리 </span>
			   ...
			   <tbody>
			       <c:forEach var="post" items="${boardList}">
			           <tr class="board-row" onclick="location.href='<c:url value='/admin/admin_boarddetail?bno=${post.bno}'/>'">
		                    ...
		                    <td>${post.writer}</td>
		                    <td><fmt:formatDate pattern = "yyyy. MM. dd" value="${post.reg_date}"/></td>
		                </tr>
			       </c:forEach>
			   </tbody>
			</table>
			<div class="paging">
				...
				<c:if test="${totalCnt!=null && totalCnt!=0}">
					<c:if test="${ph.showPrev}">
					    <a class="page" href="<c:url value="/admin/board-category-detail${ph.sc.getQueryString(ph.beginPage-1)}&bc_name=${ph.sc.bc_name}"/>">&lt;</a>
					</c:if>
					<c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
					    <a class="page ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/admin/board-category-detail${ph.sc.getQueryString(i)}&bc_name=${ph.sc.bc_name}"/>">${i}</a>
					</c:forEach>
					<c:if test="${ph.showNext}">
					    <a class="page" href="<c:url value="/admin/board-category-detail${ph.sc.getQueryString(ph.endPage+1)}&bc_name=${ph.sc.bc_name}"/>">&gt;</a>
					</c:if>
				</c:if>
			</div>
	</div>
